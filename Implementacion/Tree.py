...
class Nodo:
    ...
    def __str__(self):
        def explore(self, prefijo='',nodos=0,prof=0):        
            ...
            for etiqueta,subarbol in hijos[:-1]:
                ...
                tree, nodos, cprof = explore(subarbol,next_prefijo,nodos,prof+1)
                ret += u''.join([tree])
            if hijos:
                ...
                tree, nodos, cprof = explore(subarbol,last_prefijo,nodos,prof+1)
                ret += u''.join([tree])
            else:
                cprof = 1
                ret += u''.join([u'\n',prefijo])
            return ret,nodos+1,prof+cprof
