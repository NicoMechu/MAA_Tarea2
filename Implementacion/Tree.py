...
class Nodo:
    ...
    def __str__(self):
        def explore(self, prefijo='',nodos=0,prof=1):        
            ...
            for etiqueta,subarbol in hijos[:-1]:
                ...
                tree, nodos, _ = explore(subarbol,next_prefijo,nodos,prof+1)
                ret += u''.join([tree])
            if hijos:
                ...
                tree, nodos, prof = explore(subarbol,last_prefijo,nodos,prof+1)
                ret += u''.join([tree])
            else:
                prof += 1
                ret += u''.join([u'\n',prefijo])
            return ret,nodos+1,prof
